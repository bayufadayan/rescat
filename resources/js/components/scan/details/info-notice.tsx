...
const TEXT =
    'Informasi lokasi bersifat opsional. Anda boleh melewati bagian ini jika tidak ingin memberikannya karena terlalu privat. Informasi lokasi kami gunakan untuk melacak lokasi secara lebih akurat agar dapat memberikan rekomendasi petcare terdekat.';
        ...
        return (
            <TooltipProvider delayDuration={150}>
                <Tooltip>
                    ...
                    {/* ⬇️ tambahkan z-index lebih tinggi dari Drawer.Content (z-[60]) */}
                    <TooltipContent
                        side="bottom"
                        align="center"
                        sideOffset={8}
                        className="z-[90] max-w-sm bg-white text-neutral-800 px-3 py-2 rounded-lg shadow-lg text-sm"
                    >
                        {TEXT}
                    </TooltipContent>
                </Tooltip>
        ...
        <Popover open={open} onOpenChange={setOpen}>
            <div className="bg-neutral-600 flex gap-3 w-full items-center p-3.5 rounded-2xl opacity-75">
                ...
                <PopoverTrigger asChild>
                    <button
                        onClick={() => setOpen(v => !v)}
                        className="text-left font-medium text-sm text-white/95 line-clamp-2 w-full active:opacity-90"
                    >
                        {TEXT}
                    </button>
                </PopoverTrigger>
