    ...
    return (
        ...
        "
                style={{
                    top: '0',
                    background:
                        verdict === 'cat' ? '#16a34a' : verdict === 'noncat' || verdict === 'fail' ? '#ef4444' :
                            verdict === 'loading' ? 'rgba(0,0,0,0.55)' : 'rgba(239,68,68,1)'
                }}
                aria-live="polite"
            >
                {verdict === 'loading' && (
                ...
                {(verdict === 'noncat' || verdict === 'fail') && <X className="text-white size-12" strokeWidth={3} />}
            ...
            <div className="w-fit h-auto bg-white border-8 border-white rounded-3xl overflow-hidden p-1 mx-auto max-w-[350px] md:max-w-lg shadow">
                <div className="relative overflow-hidden rounded-2xl bg-neutral-200">
                    {hero ? (
                        ...
                        <div className="h-[360px] w-full grid place-items-center text-neutral-600">Tidak ada gambar</div>
                    )}
            ...
            <div className="mt-3 flex w-full justify-center">
                ...
                {verdict === 'cat' && (
                    <div className="flex items-center gap-2 rounded-full bg-white px-3 py-1 text-sm text-green-700 shadow">
                        ...
                        <span>Gambar adalah <b>kucing</b>{probText ? ` • ${probText}` : ''}</span>
                    </div>
                )}
                ...
                {verdict === 'noncat' && (
                    <div className="flex items-center gap-2 rounded-full bg-white px-3 py-1 text-sm text-red-700 shadow">
                        <X className="h-4 w-4" />
                        <span><b>Bukan</b> kucing{probText ? ` • ${probText}` : ''}</span>
                    </div>
                )}
